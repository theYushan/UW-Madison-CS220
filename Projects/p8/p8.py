...
original_movies = copy.deepcopy(movies)

hepburn_films = find_specific_movies(original_movies, "Katharine Hepburn")
hepburn_films
...
original_movies_1 = copy.deepcopy(movies)

wisconsin_movies = find_specific_movies(original_movies_1, "Wisconsin")
wisconsin_movies
...
def bucketize_by_genre(movies):
    ...
    genre_dict = {}
    for idx in range(len(movies)):
        for g in range (len(movies[idx]["genres"])):  
            if (movies[idx]["genres"][g] not in genre_dict):
                genre_dict[movies[idx]["genres"][g]] = []
                genre_dict[movies[idx]["genres"][g]].append(movies[idx])
            else:
                genre_dict[movies[idx]["genres"][g]].append(movies[idx])
    return genre_dict # key is genre, value is list of dictionaries
...
music = genre_dict["Music"] # music is a list of dictionaries 
for idx in range (len(music)):
    if (music[idx]["year"] > 2019 ):
        music_after_2019 += 1
music_after_2019
...
horror = genre_dict["Horror"] # list of dictionaries 

for idx in range (len(horror)):
    if (horror[idx]["rating"] > 9.0):
        horror_movies_above_9.append(horror[idx]["title"])
horror_movies_above_9
...
brad_pitt = [] #list of movie dict that has Jen
for movie in range(len(movies)):
    if "Jennifer Aniston" in movies[movie]["cast"]:
        brad_pitt.append(movies[movie])
        
ross_geller = {} #dict of list; key is genre, value is list of title
for movie in range(len(brad_pitt)):
    for g in range(len(brad_pitt[movie]["genres"])):
        if(brad_pitt[movie]["genres"][g] not in ross_geller):
            ross_geller[brad_pitt[movie]["genres"][g]] = []
            ross_geller[brad_pitt[movie]["genres"][g]].append(brad_pitt[movie]["title"])
        else:
            ross_geller[brad_pitt[movie]["genres"][g]].append(brad_pitt[movie]["title"])

max_genre = 0
for key in ross_geller:
    if max_genre == 0 or len(ross_geller[key]) > max_genre:
        jen_aniston_genre = key
        max_genre = len(ross_geller[key])
jen_aniston_genre
...
doc = [] # list of documentary movie dictionary
doc = genre_dict["Documentary"]

highest = 0 # the highest rating
for movie in range(len(doc)):
    if highest == 0 or doc[movie]["rating"] > highest:
        highest_rating = [] # list of documentary movie dictionary with the highest ratings; empty list created when new highest rate found
        highest_rating.append(doc[movie])
        highest = doc[movie]["rating"]
    elif doc[movie]["rating"] == highest:
        highest_rating.append(doc[movie])
...
for movie in range(len(highest_rating)):
    for idx in range(len(highest_rating[movie]["directors"])):
        dir_list.append(highest_rating[movie]["directors"][idx])

max_docu_rating_directors = dir_list
