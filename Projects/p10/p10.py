...
for star in stars_1_dict:
    if (stars_1_dict[star].stellar_luminosity != None):
        lum_sum += stars_1_dict[star].stellar_luminosity
        count += 1
avg_lum_stars_1 = lum_sum / count
avg_lum_stars_1
# -
grader.check("q7")
...
for star in stars_2_dict:
    if (stars_2_dict[star].stellar_age != None):
        age_sum += stars_2_dict[star].stellar_age
        count += 1
avg_age_stars_2 = age_sum / count
avg_age_stars_2
grader.check("q8")
...
for star in stars_dict:
    if stars_dict[star].stellar_radius != None:
        if biggest_star == None or stars_dict[star].stellar_radius > biggest_r :
            biggest_star = star
            biggest_r = stars_dict[star].stellar_radius
biggest_star
# -
...
for star in stars_dict:
    if star[:6] == "Kepler":
        if stars_dict[star].stellar_age != None:
            total +=  stars_dict[star].stellar_age
            count += 1
...
def get_planets(planet_file, mapping_file):
    ...
    planets_csv = process_csv(os.path.join("data", planet_file)) # read the data in 'planets_1.csv'
    planets_header = planets_1_csv[0]
...
temp_list = get_planets("planets_1.csv", "mapping_1.json")
last_five_planets_1 = temp_list[-5:]
last_five_planets_1
...
temp_list = get_planets("planets_2.csv", "mapping_2.json")
controversial_planets = []
for idx in range(len(temp_list)):
    if temp_list[idx].controversial_flag == True:
        controversial_planets.append(temp_list[idx])
controversial_planets
grader.check("q14")
...
temp = get_planets("planets_4.csv","mapping_4.json")

last_five_planets_4 = temp[-5:]
last_five_planets_4
# -
grader.check("q15")
...
csv_files = sorted(path[5:] for path in file_paths if "planet" in path)
json_files = sorted(path[5:] for path in json_file_paths)
print(csv_files)
print(json_files)
...
planets_list = get_planets('planets_1.csv', 'mapping_1.json')
for idx in range(len(csv_files)):
    if idx == 0:
        continue
    else:
        planets_list.extend(get_planets(csv_files[idx], json_files[idx]))
len(planets_list)
# -
...
for planet in planets_list:
    if planet.planet_radius == None:
        continue
    if stars_dict[planet.host_name].stellar_radius != None and stars_dict[planet.host_name].stellar_radius > 10:
...
for star in stars_dict:
    if stars_dict[star].stellar_age != None:
        if youngest_age == None or stars_dict[star].stellar_age < youngest_age:
            youngest_age = stars_dict[star].stellar_age
            youngest_star = star
for idx in range(len(planets_list)):
    if planets_list[idx].host_name == youngest_star:
        youngest_star_planets.append(planets_list[idx])
youngest_star_planets
grader.check("q20")
