...
download(os.path.join("rankings.json"), "https://git.doit.wisc.edu/cdis/cs/courses/cs220/cs220-f22-projects/-/raw/main/p12/rankings.json")

# + tags=[]
# open 'rankings.json' with pd.read_json('rankings.json') and store in the variable 'rankings'
rankings = pd.read_json(os.path.join("rankings.json"))
...
uw_madison_nat_rank = uw_madison[uw_madison["Year"] == "2021-2022"].iloc[0]["National Rank"]
uw_madison_nat_rank
...
uw_madison_avg_score = pd.Series.mean(uw_madison["Score"])
...
singapore_inst = rankings[(rankings["Year"] == "2020-2021") & (rankings["Country"] == "Singapore")]
singapore_inst
...
german_best_name = rankings[(rankings["Country"] == "Germany") & (rankings["National Rank"] == 1) & (rankings["Year"] == "2019-2020")].iloc[0]["Institution"]
german_best_name
...
highGer = rankings[(rankings["Country"] == "Germany") & (rankings["National Rank"] == 1) & (rankings["Year"] == "2019-2020")].iloc[0]["World Rank"]
us_better_than_german_best = list(rankings[(rankings["Country"] == "USA")  & (rankings["World Rank"] < highGer) & (rankings["Year"] == "2019-2020")]["Institution"])
...
hiChina = rankings[(rankings["Country"] == "China") & (rankings["Year"] == "2021-2022")]["Quality of Education Rank"].idxmin()
china_highest_qoe = rankings.iloc[hiChina]["Institution"]
# china_highest_qoe = rankings[(rankings["Country"] == "China") & (rankings["Year"] == "2021-2022") & (rankings["Quality of Education Rank"] == hiChina)].iloc[0]["Institution"]
china_highest_qoe
...
india_highest_research = list(rankings[(rankings["Country"] == "India") & (rankings["Year"] == "2020-2021") \
                                 ].sort_values(by = "Research Performance Rank").head(5)["Institution"])
...
institutions_df = rankings[rankings["Institution"].isin(list(institutions_2019_2020_2021))]
institutions_df
...
for ins in institutions_2019_2020_2021:
    dic = {}
    dic["Institution"] = ins
    dic["2019_ranking"] = institutions_df[(institutions_df["Year"] == "2019-2020") \
                                          & (institutions_df["Institution"] == ins)].iloc[0]["World Rank"]
    dic["2020_ranking"] = institutions_df[(institutions_df["Year"] == "2020-2021") \
                                          & (institutions_df["Institution"] == ins)].iloc[0]["World Rank"]
    dic["2021_ranking"] = institutions_df[(institutions_df["Year"] == "2021-2022") \
                                          & (institutions_df["Institution"] == ins)].iloc[0]["World Rank"]
    # TODO: create a new dictionary with the necessary key/value pairs
    # TODO: append the dictionary to the list
    ins_list.append(dic)
institutions_df = pd.DataFrame(ins_list)
...
improved_institutions = list(institutions_df[institutions_df["difference"] > 500]["Institution"])
...
institutions_df["abs diff"] = abs(institutions_df["difference"])
most_change = pd.Series.max(institutions_df["abs diff"])
most_change_inst = institutions_df[institutions_df["abs diff"] == most_change].iloc[0]["Institution"]
...
strict_df = institutions_df[(institutions_df["2019_ranking"] - institutions_df["2020_ranking"] > 0 ) \
                           & (institutions_df["2020_ranking"] - institutions_df["2021_ranking"] > 0 )]
strictly_improved = strict_df.shape[0]
...
alum = list(year_2020_ranking_df.sort_values(by = "Alumni Employment Rank").head(10)["Institution"])
world = list(year_2020_ranking_df.sort_values(by = "World Rank").head(10)["Institution"])
top_only_aer = [ins for ins in alum if ins not in world]
top_only_aer
...
top100_19 = list(year_2019_ranking_df.sort_values(by = "World Rank").head(100)["Institution"])
top100_21 = list(year_2021_ranking_df.sort_values(by = "World Rank").head(100)["Institution"])
top_only_2019 = [ins for ins in top100_19 if ins not in top100_21]
top_only_2019
...
top100_df = year_2020_ranking_df[year_2020_ranking_df["World Rank"] < 101]
countries = top100_df["Country"].value_counts()
top_countries = list(countries[countries >= 10].index)
top_countries
...
File1920 = open("2019-2020.html", "r")
contents = File1920.read()
bs_obj = BeautifulSoup(contents, "html.parser")
header = []
th = bs_obj.find_all("th")
for table in th:
    header.append(table.get_text())
header
...
def parse_html(filename):
    ...
    result = []
    th = bs_obj.find_all("th")
    tr = bs_obj.find_all("tr")
    header = [table.get_text() for table in th]
    for row in tr:
        values = [data.get_text() for data in row.find_all("td")]
    ...
    for ins in result:
        for key in ins:
            if ins[key] == "-":
                ins[key] = None
            elif key in ["World Rank", "National Rank", "Quality of Education Rank", "Alumni Employment Rank",\
                       "Quality of Faculty Rank", "Research Performance Rank"]:
                ins[key] = int(ins[key])
        ins["Score"] = float(ins["Score"])
        ins["Year"] = year
    return result
...
data_all = parse_html("2019-2020.html")
data_all.extend(parse_html("2020-2021.html"))
data_all.extend(parse_html("2021-2022.html"))
# print(data_all[1997]["Quality of Education Rank"])
print(data_all[2000]["Quality of Education Rank"])
# print(data_all[2001]["Quality of Education Rank"])
# print(data_all[2010]["Quality of Education Rank"])
# print(parse_html("2020-2021.html")[0]["Quality of Education Rank"])
write_json("my_rankings.json", data_all)
data_all[2000]
